...
import com.tcdt.qlnvkho.secification.QlnvQdXdungKhoHdr2Specification;
...
public class QlnvQdXdungKhoController extends BaseController {
	...
	@ApiOperation(value = "Tạo mới Quyết định xây dựng kho tàng hàng năm và thường xuyên được Tổng cục duyệt", response = List.class)
	@PostMapping(value = PathContains.HN_TX_DTCD + PathContains.URL_TAO_MOI, produces = MediaType.APPLICATION_JSON_VALUE)
	@ResponseStatus(HttpStatus.CREATED)
	public ResponseEntity<Resp> insert2(HttpServletRequest request, @Valid @RequestBody QlnvQdXdungKhoHdrReq objReq) {
		Resp resp = new Resp();
		try {
			List<QlnvQdXdungKhoDtlReq> dtlReqList = objReq.getDetail();
	...
	@ApiOperation(value = "Xoá thông tin Quyết định xây dựng kho tàng hàng năm và thường xuyên được Tổng cục duyệt", response = List.class, produces = MediaType.APPLICATION_JSON_VALUE)
	@PostMapping(value = PathContains.HN_TX_DTCD + PathContains.URL_XOA, produces = MediaType.APPLICATION_JSON_VALUE)
	@ResponseStatus(HttpStatus.OK)
	public ResponseEntity<Resp> delete2(@Valid @RequestBody IdSearchReq idSearchReq) {
		Resp resp = new Resp();
		try {
			if (StringUtils.isEmpty(idSearchReq.getId()))
				throw new Exception("Xoá thất bại, không tìm thấy dữ liệu");
	...
	@ApiOperation(value = "Tra cứu Quyết định xây dựng kho tàng hàng năm và thường xuyên được Tổng cục duyệt", response = List.class)
	@PostMapping(value = PathContains.HN_TX_DTCD + PathContains.URL_TRA_CUU, produces = MediaType.APPLICATION_JSON_VALUE)
	@ResponseStatus(HttpStatus.OK)
	public ResponseEntity<Resp> selectAll2(HttpServletRequest request, @RequestBody QlnvQdXdungKhoHdrSearchReq searchReq) {
		Resp resp = new Resp();
		try {
			int page = PaginationSet.getPage(searchReq.getPaggingReq().getPage());
			int limit = PaginationSet.getLimit(searchReq.getPaggingReq().getLimit());
			Pageable pageable = PageRequest.of(page, limit, Sort.by("id").ascending());
			...
			Page<QlnvQdXdungKhoHdr2> qhKho = qlnvQdXdungKhoHdr2Repository.findAll(QlnvQdXdungKhoHdr2Specification.buildSearchQueryHnTxdtcd(searchReq), pageable);
	...
	@ApiOperation(value = "Cập nhật Quyết định xây dựng kho tàng hàng năm và thường xuyên được Tổng cục duyệt", response = List.class)
	@PostMapping(value = PathContains.HN_TX_DTCD + PathContains.URL_CAP_NHAT, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<Resp> update2(HttpServletRequest request, @Valid @RequestBody QlnvQdXdungKhoHdrReq objReq) {
		Resp resp = new Resp();
		try {
			if (StringUtils.isEmpty(objReq.getId()))
				throw new Exception("Sửa thất bại, không tìm thấy dữ liệu");
	...
	@ApiOperation(value = "Lấy chi tiết thông tin Quyết định xây dựng kho tàng hàng năm và thường xuyên được Tổng cục duyệt", response = List.class)
	@GetMapping(value = PathContains.HN_TX_DTCD + PathContains.URL_CHI_TIET + "/{ids}", produces = MediaType.APPLICATION_JSON_VALUE)
	@ResponseStatus(HttpStatus.OK)
	public ResponseEntity<Resp> detail2(
			@ApiParam(value = "ID Quyết định xây dựng kho tàng", example = "1", required = true) @PathVariable("ids") String ids) {
		Resp resp = new Resp();
		try {
			if (StringUtils.isEmpty(ids))
	...
	@ApiOperation(value = "Trình duyệt-01/Duyệt-02/Từ chối-03/Xoá-04 Quyết định xây dựng kho tàng hàng năm và thường xuyên được Tổng cục duyệt", response = List.class)
	@PostMapping(value = PathContains.HN_TX_DTCD + PathContains.URL_PHE_DUYET, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<Resp> updateStatus2(HttpServletRequest request, @Valid @RequestBody StatusReq stReq) {
		Resp resp = new Resp();
		try {
			if (StringUtils.isEmpty(stReq.getId()))
				throw new Exception("Không tìm thấy dữ liệu");
