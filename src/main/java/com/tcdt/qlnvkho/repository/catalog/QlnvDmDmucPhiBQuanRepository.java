...
public interface QlnvDmDmucPhiBQuanRepository extends CrudRepository<QlnvDmDMucPhiBQuan, Long> {
	@Query(value = "SELECT * FROM QLNV_DM_DMUC_PHI t WHERE (:tenDMuc is null or lower(t.TEN_DINHMUC) like lower(concat(concat('%', :tenDMuc),'%'))) "
			...
			countQuery = "SELECT count(1) FROM QLNV_DM_DMUC_PHI t "
