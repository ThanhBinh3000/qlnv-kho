...
public interface QlnvDmLHinhBQuanRepository extends CrudRepository<QlnvDmLHinhBQuan, Long> {
	@Query(value = "SELECT * FROM DM_LHINH_BQUAN t WHERE (:maLHinh is null or lower(t.MA_LHINH) like lower(concat(concat('%', :maLHinh),'%'))) AND (:tenLHinh is null or lower(t.TEN_LHINH) like lower(concat(concat('%', :tenLHinh),'%'))) "
			+ " AND (:trangThai is null or t.TRANG_THAI = :trangThai)", 
			countQuery = "SELECT count(1) FROM DM_LHINH_BQUAN t "
