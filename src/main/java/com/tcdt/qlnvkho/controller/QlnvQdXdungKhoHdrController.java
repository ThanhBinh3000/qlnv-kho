...
public class QlnvQdXdungKhoHdrController extends BaseController {
	...
	public ResponseEntity<Resp> insert(@Valid HttpServletRequest request, @RequestBody QlnvQdXdungKhoHdrReq objReq) {
		Resp resp = new Resp();
	...
	public ResponseEntity<Resp> delete(@RequestBody IdSearchReq idSearchReq) {
		...
		try {
			...
			Optional<QlnvQdXdungKhoHdr> QlnvQdXdungKhoHdr = qlnvQdXdungKhoHdrRepository.findById(qhoachId);
			if (!QlnvQdXdungKhoHdr.isPresent())
				throw new Exception("Không tìm thấy dữ liệu cần xoá");
			//qlnvQhoachKhoDtlRepository.deleteByIdQhHdr(qhoachId);
			qlnvQdXdungKhoHdrRepository.deleteById(qhoachId);
		} catch (Exception e) {
			// TODO: handle exception
			resp.setStatusCode(Contains.RESP_FAIL);
			resp.setMsg(e.getMessage());
	...
	public ResponseEntity<Resp> selectAll(@RequestBody SimpleSearchReq simpleSearchReq) {
		...
		try {
			...
			Page<QlnvQdXdungKhoHdr> qhKho = qlnvQdXdungKhoHdrRepository.selectParams(simpleSearchReq.getCode(), pageable);
	...
	public ResponseEntity<Resp> update(@Valid HttpServletRequest request,@RequestBody QlnvQdXdungKhoHdrReq objReq) {
		Resp resp = new Resp();
		try {
			if (StringUtils.isEmpty(objReq.getId()))
				throw new Exception("Sửa thất bại, không tìm thấy dữ liệu");
			Calendar cal = Calendar.getInstance();
			Optional<QlnvQdXdungKhoHdr> QlnvQdXdungKhoHdr = qlnvQdXdungKhoHdrRepository.findById(Long.valueOf(objReq.getId()));
			if (!QlnvQdXdungKhoHdr.isPresent())
				throw new Exception("Không tìm thấy dữ liệu cần sửa");
			QlnvQdXdungKhoHdr dataDTB = QlnvQdXdungKhoHdr.get();
			List<QlnvQdXdungKhoDtlReq> dtlReqList = objReq.getDetail();
			objReq.setDetail(null);
			QlnvQdXdungKhoHdr dataMap = new ModelMapper().map(objReq, QlnvQdXdungKhoHdr.class);
			Authentication authentication = TokenAuthenticationService.getAuthentication(request);
			...
			List<QlnvQdXdungKhoDtl> dtlList = dataDTB.getDetailList();
			for (QlnvQdXdungKhoDtl dtl : dtlList) {
			...
			qlnvQdXdungKhoHdrRepository.save(dataDTB);
			resp.setStatusCode(Contains.RESP_SUCC);
			resp.setMsg("Thành công");
		} catch (Exception e) {
			// TODO: handle exception
